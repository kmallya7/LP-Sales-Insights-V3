...
// 2. Global Chart Styling (Matches "Inter" font & Slate theme)
Chart.defaults.font.family = "'Inter', system-ui, sans-serif";
Chart.defaults.color = '#64748b'; // slate-500
Chart.defaults.borderColor = 'rgba(148, 163, 184, 0.1)'; // faint slate border

// --- Utility Functions ---
console.log("dashboard.js loaded (Aesthetic Version)");
...
function setLoading(isLoading) {
  ...
  if (isLoading) {
    ...
    if (error) hide(error);
    if (dashboardContent) dashboardContent.classList.add("opacity-50", "pointer-events-none", "blur-sm"); // Added blur for effect
  } else {
    ...
    <section class="max-w-[1400px] mx-auto space-y-6 animate-fade-in pb-12 transition-all duration-300">
      ...
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800">
        ...
        <div class="flex flex-wrap items-center gap-2">
           <div class="flex items-center gap-2 bg-slate-50 dark:bg-slate-800 p-1 rounded-lg border border-slate-200 dark:border-slate-700">
             <button id="prev-month" class="p-2 hover:bg-white dark:hover:bg-slate-700 rounded-md text-slate-500 transition shadow-sm"><i data-feather="chevron-left" class="w-4 h-4"></i></button>
             <select id="filter-month" class="bg-transparent text-sm font-medium text-slate-700 dark:text-slate-200 focus:outline-none cursor-pointer text-center"></select>
             <input id="filter-year" type="number" class="w-16 bg-transparent text-sm font-medium text-slate-700 dark:text-slate-200 focus:outline-none text-center" value="${new Date().getFullYear()}" />
             <button id="next-month" class="p-2 hover:bg-white dark:hover:bg-slate-700 rounded-md text-slate-500 transition shadow-sm"><i data-feather="chevron-right" class="w-4 h-4"></i></button>
           </div>
           
           <button id="apply-filters" class="flex items-center gap-2 bg-slate-800 dark:bg-blue-600 hover:bg-slate-900 dark:hover:bg-blue-700 text-white px-4 py-2.5 rounded-lg text-sm font-medium transition shadow-lg shadow-slate-200 dark:shadow-none">Refresh</button>
           <button id="export-csv" class="flex items-center gap-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 px-4 py-2.5 rounded-lg text-sm font-medium transition"><i data-feather="download" class="w-4 h-4"></i></button>
        </div>
      </div>
